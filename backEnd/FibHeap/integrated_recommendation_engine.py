...
def normalize_course_code(course_code: str) -> str:
    """
    Normalize course codes: remove Z suffix but preserve L for labs.
    MATH112Z -> MATH112, MATH112ZL -> MATH112L
    """
    if not course_code:
        return course_code
...
class IntegratedRecommendationEngine:
    ...
    1. Fibonacci heap for efficient top-K extraction
    2. Actual major requirements from track_graduation.py
    3. User preferences from PreferencesPage
    """
    ...
    def _get_remaining_requirements(self, completed: Set[str], major_reqs: Dict) -> Tuple[Set[str], List[Dict]]:
        """Determine which major requirements still need to be fulfilled"""
        # Must courses
    ...
    def _parse_time_unavailable(self, time_unavailable: List[Dict]) -> List[Tuple]:
        """Parse unavailable time blocks from preferences"""
        blocks = []
    ...
    def _has_time_conflict(self, course: Dict, unavailable_blocks: List[Tuple]) -> bool:
        """Check if course conflicts with user's unavailable times"""
    ...
    def _check_prerequisites(self, prereqs: List[List[str]], completed: Set[str]) -> bool:
        ...
        prereqs format: [[OR group 1], [OR group 2], ...] (AND between groups)
        """
