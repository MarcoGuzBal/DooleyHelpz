...
// Bucketing parser: returns { incoming, emory }
...
export default function TranscriptParserPage() {
  ...
  // codes-only parsing + posting state
  const [courseCodes, setCourseCodes] = useState<string[]>([]); // UI display can remain flat
  ...
  async function renderPreview(item: UploadedItem) {
    ...
    await page.render({
      canvasContext: ctx,
      viewport: scaled,
      canvas, // required by this pdfjs-dist build
    }).promise;
  }
              ...
              {courseCodes.length > 0 ? (
                <>
                  <div className="mb-3 text-sm text-zinc-600">
                    Found <strong>{courseCodes.length}</strong> unique codes
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                    {courseCodes.map((code) => (
                      <div
                        key={code}
                        className="rounded-md border border-zinc-200 bg-white px-2.5 py-2 text-sm shadow-sm"
                      >
                        {code}
                      </div>
                    ))}
